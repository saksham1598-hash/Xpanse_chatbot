...
- ECS Pull Phase - At the container initialization time vectorstore will be connected with the ECS container so that the retriever can pull out the relevant vectors based on a user query.
...
|-----------------|------------------------|---------------------------------------------------------------|
| **Compute**     | Amazon ECS + Fargate   | Run containerized chatbot APIs without managing servers                         |
...
| **Observability** | CloudWatch + SNS     | Monitor logs, set up alarms, and send notifications                            |
| **Security**    | IAM Roles, SGs         |  Enforce least-privilege access and manage traffic via network firewall rules |
**Application Layer** |	Flask + Gunicorn	|/query & /health endpoints for RAG APIs|
**Test Environment**| Amazon EC2s| Create test environment to mimic production
...
- **Service**: Maintains tasks for HA, auto-scales based on CPU/memory.
...
- **Storage**:  
  ...
  - Debug & error logs  
  - Usage metrics  
  - (Optional) Langfuse ( for LLM tracing, observability)
...
---
### **2.8 Flask-wrapped API endpoints defined in `app.py`**
...
This section outlines the Continuous Integration, Continuous Deployment, and MLOps processes that could be used to move the chatbot from development through test to production.
...
---
#### 2. **Vectorstore Reference Validation**
...
- Build the main Docker image using:
  ...
  - VECTOR_STRATEGY: passed-in input (e.g., colocated, s3)
    - Download vectorstore from S3 during image building ( eg colocated)
...
- Ping the /health endpoint of the app to verify runtime status.
...
1. **Connect & Deploy on EC2-1**  
   - SSHs into **EC2-1** and pull the latest code from the test branch
      - install new dependencies and add new features
...
CloudWatch Logs
Logs from both ECS and EC2 should be streamed to CloudWatch, including app logs, ETL output, and service restarts. Useful for centralized debugging and traceability.
...
These triggers would alert when resource limits or error patterns are detected.

- SNS Alerts send real-time notifications via SNS to Slack or email to inform the team about failures or performance issues.

---
### **3.5. Containerization & Resource Management**
**Dockerfile Overview** 
The Dockerfile will be based on a Python 3.10 image.
...
- It will use supervisord as the entrypoint to run and monitor app processes.
...
- **Image & Container Info**:  
  - The container will be launched using the latest image from ECR
  - It will expose port 8080 for incoming traffic.

- **Environment Variables**:  
  These will determine how the chatbot loads and use the vectorstore:
...
- **Logging**:  
  - All logs will stream to **CloudWatch Logs** under that ecs group
...
- **Fargate Settings**:
  ...
  - Allocated resources: **CPU** and **RAM**
